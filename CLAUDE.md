...
Precept is a **cognitive enforcement layer for AI software development** — not an AI coding assistant. Its purpose is to force AI agents to *understand* a software system (business logic, architecture, UX patterns, reusable assets, impact) *before* generating or modifying code.
...
# Run CLI
uv run precept --help
uv run precept scan /path/to/repo
uv run precept analyze "add OTP login" --repo /path/to/repo
uv run precept impact "fix payment scan 501" --repo /path/to/repo
uv run precept conventions /path/to/repo
uv run precept assets payment --repo /path/to/repo
uv run precept pack payment          # show built-in cognition pack

# Memory commands
uv run precept memory list --repo /path/to/repo
uv run precept memory recall "OTP policy" --repo /path/to/repo
uv run precept memory decide payment "Use idempotency keys" --body "All payment calls require idempotency key" --repo /path/to/repo
uv run precept memory forget <entry-id> --repo /path/to/repo

# Start MCP server (stdio — for Claude Code integration)
uv run precept mcp --repo /path/to/repo

# Start MCP server (SSE — for HTTP clients)
uv run precept mcp --sse --port 8765 --repo /path/to/repo

# Start REST API
uv run uvicorn precept.api.main:app --reload --port 8000
...
Six layers, each in its own package under `src/precept/`:
...
All tools live in `src/precept/mcp/server.py`. AI agents call these via MCP before coding:
...
- **Multi-repo aware** — `repo_path` is always explicit; tools can target any repo, not just the one Precept lives in.

### Phase 3 — Workspace + Graph export + Approval queue

- `get_workspace_context(workspace_path)` — full multi-repo overview from precept.toml
...
Defined in `src/precept/workspace/`. Driven by `precept.toml` at workspace root:
...
CLI: `precept workspace init | scan | impact <repo> -c "..." | graph [mermaid|dot|react_flow]`

### Graph export

`src/precept/graph/exporter.py` — `GraphExporter` produces:
...
`src/precept/approval/queue.py` — stored in `.precept/approvals.json`.

Flow: AI calls `request_approval()` → stores as `pending` → human runs `precept approval approve <id>` → AI polls `check_approval()` → proceeds only on `approved`.

CLI: `precept approval list | show <id> | approve <id> | reject <id> --reason "..."`
