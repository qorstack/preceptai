...
```text
~/.precept/                              ← user-level central store
  workspaces/
    my-product/
...
Override default `~/.precept` ด้วย env var: `PRECEPT_HOME=/custom/path`
...
# → ~/.precept/workspaces/my-product/

# 2. แก้ workspace.toml ใน ~/.precept/workspaces/my-product/
...
# → สร้าง .precept/config.toml (commit เข้า git)

# 4. ใช้งานปกติ — Precept resolve workspace อัตโนมัติ
precept analyze "rename users.email"
# → อ่าน memory จาก ~/.precept/workspaces/my-product/memory.json
...
**Precept ไม่มี LLM ใน core** — AI agent ของคุณ (Claude/Cursor/etc.) เป็นคน synthesize:

1. AI call `get_domain_knowledge("billing")`
2. Precept ส่ง raw entries + synthesis status (stale หรือ fresh)
...
→ ได้คุณภาพ LLM โดยที่ Precept ไม่มี dependency LLM (ไม่ต้อง API key, ไม่มีค่าใช้จ่าย, ไม่ vendor lock-in)
...
AI ทำให้ strict ขึ้นได้ (เลื่อนขวา) ตาม historical context AI **ห้าม** ทำให้หลวมลง (เลื่อนซ้าย) Precept decision คือ floor

## Backward compatibility

ของเดิมยังใช้ได้หมด:

- ถ้า repo ไม่มี `.precept/config.toml` → fallback ใช้ `<repo>/.precept/memory.json` แบบเดิม
- ถ้า `precept.toml` อยู่ root folder ที่ครอบ repo → ใช้ pattern เดิม (siblings layout)
...
| Command | หน้าที่ |
| --- | --- |
| `precept workspace create <name>` | สร้าง central workspace ที่ `~/.precept/workspaces/<name>/` |
| `precept workspace list` | list central workspaces ทั้งหมด |
| `precept link <workspace>` | link repo ปัจจุบันไปยัง central workspace |
| `precept unlink` | ลบ `.precept/config.toml` |
| `precept migrate` | ย้าย legacy `<repo>/.precept/memory.json` → central |

## Files ที่สร้างใน Phase 4.A

| File | หน้าที่ |
| --- | --- |
| [src/precept/paths.py](../src/precept/paths.py) | central path resolver (cross-platform, PRECEPT_HOME env) |
| [src/precept/link/config.py](../src/precept/link/config.py) | LinkConfig + read/write `.precept/config.toml` |
| [src/precept/link/resolver.py](../src/precept/link/resolver.py) | walk up from cwd → resolve workspace (or fall back to legacy) |
...
| File | Change |
| --- | --- |
| [src/precept/memory/store.py](../src/precept/memory/store.py) | `create_store()` resolves workspace automatically |
| [src/precept/approval/queue.py](../src/precept/approval/queue.py) | `get_queue()` resolves workspace automatically |
| [src/precept/workspace/config_loader.py](../src/precept/workspace/config_loader.py) | `load()` falls back to central; new `load_central()` |
| [src/precept/cli/main.py](../src/precept/cli/main.py) | new commands: `workspace create/list`, `link`, `unlink`, `migrate` |
...
แก้ด้วย: persistent scan cache ที่ `~/.precept/workspaces/<name>/scans/<repo>.json` — รัน scan บน CI หรือเครื่อง tech lead 1 ครั้ง, push เข้า central → dev คนอื่นใช้ cache นั้นได้แม้ไม่ clone repo นั้นๆ

## Phase 4.C — Git sync (planned)

```bash
# ผูก ~/.precept/workspaces/my-product/ กับ git remote
precept sync init --remote git@github.com:company/precept-shared.git
...
# → analyze_intent resolved workspace 'my-product' from .precept/config.toml
...
Junior ไม่ต้องรู้เลยว่า memory/topology อยู่ที่ไหน — Precept resolve อัตโนมัติ
