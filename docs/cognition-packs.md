...
Built-in domain knowledge bundles. When AI works on a domain, Precept injects the relevant pack into the cognition report — so the AI knows the rules even if your codebase doesn't spell them out.
...
Phase 4 will allow custom packs at `~/.precept/workspaces/<name>/packs/` so teams can codify their own conventions. For now, use [`memory decide`](cli.md#memory-decide) to record team-specific rules.

## Contributing a pack

Built-in packs live in [src/precept/packs/builtin.py](../src/precept/packs/builtin.py). Open a PR adding a new `CognitionPack(...)` entry + a test in `tests/test_packs.py`.
