...
| Task | Why |
|---|---|
| `precept init` command | auto-gen `precept.toml` |
...
- [x] `src/precept/paths.py` — central path resolver + PRECEPT_HOME env
- [x] `src/precept/link/` — per-repo `.precept/config.toml` + auto-resolver
...
- [ ] Persistent scan cache at `~/.precept/workspaces/<name>/scans/<repo>.json`
- [ ] `WorkspaceScanner` reads cached scan when repo not on disk
- [ ] `ImpactAnalyzer` cross-repo answers work even with single clone
- [ ] CLI: `precept workspace scan --all --persist` (for CI/tech lead)

## 🔵 Phase 4.C — Git sync (not started)

- [ ] `src/precept/sync/git_sync.py` — pull/push central store to git remote
- [ ] CLI: `precept sync init/pull/push/status`
...
| Sprint 2 | `precept init`, complete REST API, CI/CD, publish PyPI |
