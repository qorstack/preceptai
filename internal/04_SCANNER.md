...
📂 [src/precept/scanner/](../src/precept/scanner/)
...
- Key = `repo_path`
- Invalidate via `refresh_scan(repo_path)` MCP tool หรือ `precept scan` CLI
...
```python
# Python
from precept.scanner import RepoScanner
...
2. `uv run precept scan .` → เห็นภาพรวมใน 3 วินาที (architecture, domains, conventions)
