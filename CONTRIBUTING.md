...
- What happened
- Python version + OS
- Output of `precept scan .` if relevant
...
Precept has 6 layers under `src/precept/`:
...
Built-in packs live in [src/precept/packs/builtin.py](src/precept/packs/builtin.py). Each pack has:
...
```bash
# Bump version in pyproject.toml + src/precept/__init__.py
git tag v0.X.0
git push --tags
