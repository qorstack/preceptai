"""
Per-repo link config — knowai.config at the root of each project repo.

The same file may also carry a [database] section (read by knowai/__init__.py).
...
def save_link(config: LinkConfig, repo_path: str | Path = ".") -> Path:
    """Write knowai.config inside repo_path, preserving any [database] section."""
    path = repo_link_config_path(repo_path)
