"""Load and save precept.toml workspace config."""

from __future__ import annotations

from pathlib import Path

from precept.workspace.schema import RepoConfig, RepoDependency, RepoRole, WorkspaceConfig

_DEFAULT_FILENAME = "precept.toml"


def load(workspace_path: str | Path = ".") -> WorkspaceConfig:
    """
    Load precept.toml from workspace_path. Returns empty config if not found.

    Resolution order:
    1. If workspace_path/precept.toml exists → load that (legacy/sibling layout)
    2. Else if workspace_path (or ancestor) is a linked repo → load central
       ~/.precept/workspaces/<name>/workspace.toml
...
def load_central(workspace_name: str) -> WorkspaceConfig:
    """Load workspace.toml directly from ~/.precept/workspaces/<name>/."""
...
def save(config: WorkspaceConfig, workspace_path: str | Path = ".") -> Path:
    """Serialize config to precept.toml + write any embedded repos as per-file
...
def init(workspace_path: str | Path = ".", name: str = "") -> WorkspaceConfig:
    """Create a minimal precept.toml in workspace_path."""
    root = Path(workspace_path).resolve()
...
def register_repo_in_workspace(
    ...
    """
    from precept.paths import workspace_toml_path
...
def _serialize(config: WorkspaceConfig) -> str:
    ...
    `precept init` calls from different devs avoid colliding on the same file.
    """
    lines = [
        f'name = "{config.name}"',
