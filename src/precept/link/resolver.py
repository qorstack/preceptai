...
Resolution strategy:
1. Look for precept.config in repo_path (or any parent up to FS root).
2. If found, return the workspace name + central paths.
3. If not found, return None — caller can fall back to legacy
   per-repo `<repo>/.precept/memory.json` behavior.
"""
...
from precept.link.config import LinkConfig, load_link
from precept.paths import (
...
def _find_link_upwards(start: Path) -> tuple[Path, LinkConfig] | None:
    """Walk up from `start` looking for precept.config with `workspace` set."""
    p = start.resolve()
    while True:
        cfg = load_link(p)
        if cfg is not None:
...
def resolve_workspace_or_legacy(repo_path: str | Path = ".") -> tuple[Path, Path, str]:
    ...
    Resolution order:
    1. If repo (or ancestor) has `precept.config` → "central" — use the
    ...
    3. Otherwise "legacy" — per-repo `.precept/memory/`.
    """
