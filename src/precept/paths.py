"""
Central path resolver for Precept user-level storage.
...
Environment override: PRECEPT_HOME can override ~/.precept
"""
...
def precept_home() -> Path:
    """User-level Precept home. Honors PRECEPT_HOME env var."""
    env = os.environ.get("PRECEPT_HOME")
    if env:
        return Path(env).expanduser().resolve()
    return Path.home() / ".precept"


def workspaces_root() -> Path:
    return precept_home() / "workspaces"
...
def workspace_dir(name: str) -> Path:
    ...
    Resolution: checks the registry (~/.precept/registry.toml) first — if
    ...
    back to the default ~/.precept/workspaces/<name>/.

    Does NOT create the directory.
    """
    # Local import to avoid circular dependency: registry.py imports precept_home.
    try:
        from precept.registry import get_path
...
def list_workspaces() -> list[str]:
    """All workspace names present in ~/.precept/workspaces/."""
...
def repo_link_config_path(repo_path: str | Path) -> Path:
    """Per-repo precept.config path (workspace link + optional DB override)."""
    return Path(repo_path) / "precept.config"
