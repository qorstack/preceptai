...
EXISTS / DO $$ guards make repeats free. Team workflow:
    ...
    3. precept init                # writes precept.config + MCP config
...
from precept.memory.store import MemoryStore
...
def _load_schema_sql() -> str:
    return resources.files("precept.memory.sql").joinpath("schema.sql").read_text(encoding="utf-8")
...
class PostgresMemoryStore(MemoryStore):
    ...
    def _bootstrap(self) -> None:
        ...
        with self._pool.connection() as conn:
            ...
            # type resolves regardless of which schema precept's tables are in.
