...
def load_workspace_skills(workspace_name: str) -> list[Skill]:
    ...
    1. Built-in skills (auth, billing/payment, otp, webhook, order, etc.)
       — ship with precept as sensible defaults derived from cognition packs.
...
def load_builtin_skills() -> list[Skill]:
    """
    Skills shipped with precept — derived from BUILTIN_PACKS at runtime
    ...
    """
    try:
        from precept.packs.builtin import BUILTIN_PACKS
    except Exception:
        return []
