...
Goal: let users verify, after the fact, that the AI actually called Precept
tools — and which ones. Each tool call appends one JSON line to:

    <repo>/.precept/audit.log

To keep the file bounded, we cap at PRECEPT_AUDIT_MAX lines (default 500) —
...
def _audit_path(repo_path: str | Path) -> Path:
    return Path(repo_path) / ".precept" / "audit.log"


def _max_lines() -> int:
    raw = os.environ.get("PRECEPT_AUDIT_MAX", "")
...
def log(repo_path: str | Path, tool: str, **args: Any) -> None:
    ...
    Set PRECEPT_AUDIT_DISABLE=1 to skip writing the per-repo .precept/audit.log
    entirely (the Postgres memory_audit_log still tracks every memory change).
    """
    if os.environ.get("PRECEPT_AUDIT_DISABLE", "").lower() in ("1", "true", "yes"):
        return
    try:
