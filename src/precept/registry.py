...
Stored at ~/.precept/registry.toml so the CLI can find a workspace by name
no matter where its files live (a cloned knowledge repo, a custom path, or
the default ~/.precept/workspaces/<name>/).

Schema (TOML):
    [workspaces]
    tutorial = "C:/Me/PersonalCoding/tutorial-precept/tutorial-precept-knowledge"
    other    = "/home/me/.precept/workspaces/other"

Resolution order in paths.workspace_dir(name):
    1. registry entry (if present and the path still exists)
    2. ~/.precept/workspaces/<name>/  (default)
"""
...
def _registry_path() -> Path:
    return precept_home() / "registry.toml"
...
def _save_raw(entries: dict[str, str]) -> Path:
    ...
    lines = [
        "# Precept workspace registry — maps workspace name to on-disk path.",
        "# Managed by `precept init` and `precept workspace register`.",
        "",
        "[workspaces]",
    ]
