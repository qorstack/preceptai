...
- **Opt-out friendly.** Set `PRECEPT_AUTO_SYNC=0` to disable globally.
...
Public API:
    ...
    full_sync(workspace_dir, message, files=...)
                           — pull → push. Used by `precept sync`.
...
def sync_enabled() -> bool:
    """Honor PRECEPT_AUTO_SYNC env var. Default: enabled."""
    val = os.environ.get("PRECEPT_AUTO_SYNC", "1").strip().lower()
...
def _preflight(workspace_dir: str | Path) -> tuple[Path, SyncResult | None]:
    ...
    if not sync_enabled():
        return Path(workspace_dir), _skip("PRECEPT_AUTO_SYNC=0")
...
def full_sync(
    ...
    Used by both the `precept sync` CLI command and the post-write hooks in
    `precept memory decide`, `precept approval ...`, and `precept init`.
    """
    ...
    msg = message or "chore(precept): auto-sync"
...
# Result is written to `<workspace>/.precept-sync-status.json` so
# `precept doctor` can show what happened.
...
def _spawn_detached_sync(p: Path, message: str, files: list[str] | None) -> None:
    """Launch `python -m precept.sync.auto _bg_sync <args>` detached."""
    import json as _json
    import sys
    payload = _json.dumps({"path": str(p), "message": message, "files": files or []})
    cmd = [sys.executable, "-m", "precept.sync.auto", "_bg_sync", payload]
    kwargs: dict = dict(
...
def _record_status(
    ...
    """Drop the result of a sync cycle so `precept doctor` can read it."""
    import json as _json
    from datetime import datetime, timezone
    p = Path(workspace_dir) / ".precept-sync-status.json"
    try:
        p.write_text(
            _json.dumps({
...
def _bg_main(argv: list[str]) -> int:
    """Entry point for `python -m precept.sync.auto _bg_sync <json>`."""
    import json as _json
